...
export default function EditProfilePage() {
  ...
  const [lastDonationDate, setLastDonationDate] = useState<Date | undefined>(new Date("2024-04-29")) // Mock date
  const [isDialogOpen, setIsDialogOpen] = useState(false)

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    // In a real application, you would send this updated data to a server.
    console.log({ fullName, bloodGroup, phoneNumber, university, city, lastDonationDate })
  ...
  return (
    <div className="min-h-screen bg-app-bg flex flex-col">
      ...
      <header className="flex items-center p-4 pt-6">
        ...
        <h1 className="flex-1 text-center text-xl font-bold text-gray-800">Edit Profile</h1>
      ...
      <main className="flex-1 p-4 pb-20">
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <PopoverTrigger asChild>
                <Button
                  ...
                  className={cn(
                    ...
                    !lastDonationDate && "text-app-placeholder",
                  )}
                >
                  {lastDonationDate ? format(lastDonationDate, "PPP") : <span>Select last donation date</span>}
          ...
          <Button
            type="submit"
            className="w-full bg-app-red hover:bg-app-red/90 text-white text-lg font-semibold py-6 rounded-xl shadow-md mt-8 transition-all duration-200 ease-in-out"
          >
            Save Changes
          </Button>
        </form>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px] bg-white rounded-lg p-6">
          <DialogHeader>
            <DialogTitle className="text-center text-2xl font-bold text-gray-800">Profile Updated!</DialogTitle>
